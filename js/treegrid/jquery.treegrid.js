...
(function($) {
    ...
    var methods = {
        ...
        initEvents: function() {
            ...
            $this.on("collapse", function() {
                var $this = $(this);
                $this.removeClass('treegrid-expanded'); // 这个样式treegrid-expanded只是用来判断树的结点是否展开的，并没有任何css定义
        ...
        initSettingsEvents: function() {
            ...
            $this.on("collapse", function() {
                ...
                if (typeof($this.treegrid('getSetting', 'onCollapse')) === "function") {
                    $this.treegrid('getSetting', 'onCollapse').apply($this); // 回调监听钩子
                }
            });
            //Default behavior on expand
            $this.on("expand", function() {
        ...
        render: function() {
            return $(this).each(function() {
                ...
                if ($this.treegrid('isOneOfParentsCollapsed')) {
                    $this.hide();
                    //$this.slideUp(); // 动态收缩（做的不好）
                } else {
                    $this.show();
                    //$this.slideDown(); // 动态滑开（做的不好）
                }
                if (!$this.treegrid('isLeaf')) {
    ...
    // 扩展jquery的插件treegird的构造函数
    $.fn.treegrid = function(method) {
        if (methods[method]) {
            // 如果是传入了方法，则是调用方法，从参数中剪切掉1个，直接回调这个方法
    ...
    $.fn.treegrid.defaults = {
        ...
        // 获取父节点id
        getParentNodeId: function() {
            var template = /treegrid-parent-([A-Za-z0-9_-]+)/; // 这里正则匹配若干个数字或字母
            if (template.test($(this).attr('class'))) {
                return template.exec($(this).attr('class'))[1];
            }
            return null;
